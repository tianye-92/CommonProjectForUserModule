...
public class AuthController {
    ...
    @PostMapping(value = "/loginForOMS")
    @ApiOperation("OMS客户端系统登录调试授权 最高权限用户： admin/123456")
    @ApiImplicitParams({
    ...
    public void loginForOMS(String username, String password) {
        if (StringUtils.isBlank(username) || StringUtils.isBlank(password)) {
            throw new GlobalException(ResponseCodeEnum.RETURN_CODE_100004);
        }
        String newPassword = password + "-" + username;
        Object login = remoteUserService.loginForPC(username, new BASE64Encoder().encode(newPassword.getBytes()), "1");
        commonVerify(login);
    }
    ...
    public void loginForPC(String username, String password) {
        ...
        String newPassword = password + "-" + username;
        Object login = remoteUserService.loginForPC(username, new BASE64Encoder().encode(newPassword.getBytes()), "0");
        commonVerify(login);
    }
    ...
    public void loginForPDA(String username, String password, String warehouseCode, String warehouseName) {
        ...
        String newPassword = password + "-" + username;
        Object login = remoteUserService.loginForPDA(username, new BASE64Encoder().encode(newPassword.getBytes()), warehouseCode, warehouseName, "0");
        commonVerify(login);
    }
    ...
    private void commonVerify(Object object) {
        ...
        String platformType = response.getPlatformType();
