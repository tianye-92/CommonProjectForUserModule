...
public class ExcelRead<T> {
	 ...
	 * @param suffix    文件后缀
	 * @param clazz     [对应的实例]
	 * @param selected  下拉选择数据
	 * @param setFormat [true/false]是否需要设置Excel的格式
	 * @param request
	 * @param response
	 */
	public void exportExcel(String fileName, String path, String suffix, Class<T> clazz, Map<String, String[]> selected, boolean setFormat, HttpServletRequest request, HttpServletResponse response) {
		try {
			if (!setFormat)
				ExcelUtil.downloadTemplate(fileName, suffix, path, request, response);
			this.clazz = clazz;
			this.suffix = suffix;
			checkType(new ClassPathResource(path).getInputStream());
			downloadTemplate(fileName, selected, request, response);
		} catch (Exception e) {
			logger.error("模板读取: ", e);
		}
	}
	...
	public InputStream getResultStream(List<? extends ResultExcelModel> data) throws IOException {
		if (isErrorExcel && indexRows > 0) {
			...
			throw new RuntimeException("错误数据为空!");
		}
	}
	...
	private void downloadTemplate(String fileName, Map<String, String[]> selected, HttpServletRequest request, HttpServletResponse response) throws Exception {
		setMap(false);
		for (String k : key.keySet()) {
			ClassModel cm = key.get(k);
			if (selected.containsKey(cm.getSelectedKey())) {
				CellRangeAddressList cral = new CellRangeAddressList(1, 100000, Integer.valueOf(cm.getColumnIndex()) - 1, Integer.valueOf(cm.getColumnIndex()) - 1);
