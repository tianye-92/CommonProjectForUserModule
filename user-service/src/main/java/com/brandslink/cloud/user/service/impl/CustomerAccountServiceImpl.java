...
public class CustomerAccountServiceImpl implements ICustomerAccountService {
    ...
    public void addAccount(AddOrUpdateCustomerAccountRequestDTO request) {
        ...
        CustomerUserInfo customerInfo = customerUserInfoMapper.selectByContactWay(request.getContactPhone());
        if (null != customerUserInfo) {
            throw new GlobalException(ResponseCodeEnum.RETURN_CODE_100003);
        } else if (null != customerInfo) {
            throw new GlobalException(ResponseCodeEnum.RETURN_CODE_100003.getCode(), "该手机号:" + request.getContactPhone() + ",已注册，请更换手机号");
        }
